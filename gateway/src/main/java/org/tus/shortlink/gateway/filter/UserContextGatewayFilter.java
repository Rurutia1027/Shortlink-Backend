...
public class UserContextGatewayFilter implements GlobalFilter, Ordered {
    ...
    public Mono<Void> filter(ServerWebExchange exchange, GatewayFilterChain chain) {
        ...
        if (StrUtil.isNotBlank(token)) {
            ...
            UserInfoDTO userInfoDTO = resolveUserInfo(token);

            if (userInfoDTO != null) {
                log.debug("User context resolved: {} (userId: {})",
                        userInfoDTO.getUsername(), userInfoDTO.getUserId());

                // Add user information to request headers for downstream services
                ServerHttpRequest modifiedRequest = request.mutate()
                        .header(USER_ID_HEADER, userInfoDTO.getUserId() != null ?
                                userInfoDTO.getUserId() : "")
                        .header(USERNAME_HEADER, userInfoDTO.getUsername() != null ?
                                userInfoDTO.getUsername() : "")
                        .header(REAL_NAME_HEADER, userInfoDTO.getRealName() != null ?
                                userInfoDTO.getRealName() : "")
                        .build();

                return chain.filter(exchange.mutate().request(modifiedRequest).build());
            } else {
                log.debug("No user information found for token. Token may be invalid or " +
                        "expired.");
            }
        } else {
            log.debug("No token found in request. Request URI: {}, Metohd: {}",
                    request.getURI().getPath(), request.getMethod());
        }

        // Continue after chain even if no token/user found
     ...
     *
     * <p><strong>TODO: </strong> After migrating to Identity Module, this filter should be
     * removed and replaced with a simpler filter that calls Identity Service.
     */
    @Override
    public int getOrder() {
    ...
    private UserInfoDTO resolveUserInfo(String token) {
        ...
        try {
            // Step 1: Get username from reverse mapping (toke -> username)
            ...
            if (username == null || username.isBlank()) {
                log.debug("No username found for token (token may be invalid or expired). " +
            ...
            log.error("Error resolving user info ");
            return null;
        }
    }
     ...
     * This avoids Gateway depending on admin module.
     */
    @Data
